...
export const analyzeProductImage = async (base64Image: string): Promise<ProductAnalysis[]> => {
  try {
    ...
    // 3. Define the Prompt (We force JSON format here since v1 doesn't support schema mode)
    ...
    **IMPORTANT**: 
    ...
    // We remove the 'generationConfig' JSON schema to prevent 400 Errors.
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        // REMOVED: generationConfig (This was causing the 400 error)
      })
    });
    ...
    // 5. Clean the response (Remove markdown if the AI adds it despite instructions)
